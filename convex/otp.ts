...
import { PasswordResetEmail, VerificationEmail } from "../src/features/auth/templates";
...
export const otpVerificationEmail = Email({
  ...
  async sendVerificationRequest({ identifier: email, provider, token, expires }) {
    const expMs = typeof expires === "number" ? (expires < 1e12 ? expires * 1000 : expires) : +expires;

    const minutesUntilExpiry = Math.max(0, Math.ceil((expMs - Date.now()) / 60000));
