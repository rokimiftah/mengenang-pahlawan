// src/features/heroes/ui/HeroFilters.tsx
...
export function SearchFilterLauncher({ onApply }: { onApply: (v: SearchFilter) => void }) {
  ...
  const measureHeader = useCallback(() => {
    ...
    setHeaderRect({ top: r.top, left: r.left, width: r.width, height: r.height });
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    const el = document.querySelector("header");
    const ro = el && "ResizeObserver" in window ? new ResizeObserver(measureHeader) : null;
  ...
  useEffect(() => {
    const calc = () => {
      ...
      setInlineWidth(vw >= 1024 ? 260 : Math.max(200, Math.min(260, Math.round(vw * 0.35))));
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => e.key === "Escape" && closeSmooth();

  const ringClass = useMemo(() => (open || q ? "ring-2 ring-[#ed1c24]" : "ring-1 ring-black/10"), [open, q]);
