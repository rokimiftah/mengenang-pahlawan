// src/features/heroes/ui/HeroShelf.tsx
...
const ERA_OPTIONS = ["Perang Kolonial", "Pergerakan Nasional", "Pendudukan Jepang", "Revolusi & Orde Lama"] as const;
	...
	return (
		<button
			...
			className={[
				...
				disabled ? "cursor-not-allowed opacity-30" : "cursor-pointer hover:bg-black/70",
				className || "",
			].join(" ")}
		>
	...
	return (
		...
		>
			...
			<div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-2">
				<div className="truncate text-left text-xs font-semibold text-white/95 sm:text-sm">{hero.name}</div>
			</div>
		</button>
	);
}
...
function writeStored(era: string, page: number) {
	try {
		...
		sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ era, pageByEra, v: 1, t: Date.now() }));
	} catch {}
}
...
export function HeroShelf() {
	...
	const initialEra = ((): (typeof ERA_OPTIONS)[number] => {
		const e = stored?.era;
		return (ERA_OPTIONS as readonly string[]).includes(e) ? (e as any) : "Perang Kolonial";
	})();
	const [era, setEra] = useState<(typeof ERA_OPTIONS)[number]>(initialEra);

	const [cols, setCols] = useState<number>(() => calcCols(typeof window !== "undefined" ? window.innerWidth : 1024));
	useEffect(() => {
		const onResize = () => {
	...
	const heroes = HeroesApi.useHeroesList({ q: filters.q, era, limit: 200 });
	...
	const storedPage = typeof stored?.pageByEra?.[initialEra] === "number" ? stored.pageByEra[initialEra] : 0;
	...
	useEffect(() => {
		if (prevEraRef.current !== era) {
			...
			const st = readStored();
			const p = typeof st?.pageByEra?.[era] === "number" ? st.pageByEra[era] : 0;
			setPage(p);
		}
	}, [era]);
	...
	const setTrackTransform = useCallback(
		(pageLike: number, dxPx = 0, withTransition = true) => {
			...
			if (withTransition !== animEnabledRef.current) {
				...
				el.style.transition = withTransition ? "transform 300ms ease-out" : "none";
			}
	...
	const onPointerUp = (e: React.PointerEvent) => {
		...
		if (dx <= -threshold && canNext) next = Math.min(pages.length - 1, startPageRef.current + 1);
		else if (dx >= threshold && canPrev) next = Math.max(0, startPageRef.current - 1);
	...
	const onWheel = (e: React.WheelEvent) => {
		...
		const unit = e.deltaMode === 1 ? 16 : e.deltaMode === 2 ? viewportRef.current?.clientHeight || 800 : 1;
	...
	return (
		<div className="flex h-full min-h-0 w-full flex-col">
			<Tabs.Root className="flex h-full min-h-0 w-full flex-col" value={era} onValueChange={(v) => setEra(v as any)}>
				<Tabs.List
					className={[
				...
				<Tabs.Content value={era} className="mt-4 min-h-0 flex-1 focus:outline-none">
					<div className="flex h-full min-h-0 w-full items-stretch gap-4">
						<div className="relative h-full min-h-0 w-full flex-1">
							...
							>
								<div ref={trackRef} className="flex h-full min-h-0" style={{ transform: `translateX(-${page * 100}%)` }}>
									{heroes === undefined && (
										<div className="h-full min-h-0 w-full shrink-0">
											...
											>
												{Array.from({ length: cols }).map((_, i) => (
													<div key={i} className="h-full min-h-0 animate-pulse rounded-2xl bg-zinc-800/70" />
												))}
											</div>
										</div>
									)}
													...
													>
														...
														{group.length < cols &&
															Array.from({ length: cols - group.length }).map((_, i) => (
																<div key={`f-${i}`} className="invisible h-full min-h-0" />
															))}
													</div>
												</div>
											))
