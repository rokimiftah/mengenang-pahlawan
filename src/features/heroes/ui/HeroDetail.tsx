// src/features/heroes/ui/HeroDetail.tsx
...
import { HeroesApi } from "@features/heroes";
...
function Choice({
  ...
  const base = "w-full text-left rounded-lg border px-3 py-2 text-sm transition cursor-pointer text-neutral-600";
  ...
  return (
    <button type="button" className={cls} onClick={() => onPick(option.id)} disabled={locked} aria-pressed={selected}>
      {option.text}
    </button>
  );
}

function RecommendPanel({ slug, onClose }: { slug: string; onClose: () => void }) {
  const recommend = HeroesApi.useRecommendHeroes();
  ...
  return (
    <div className="flex h-[560px] max-h-[85vh] w-[720px] max-w-[95vw] flex-col overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-black/10">
      <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
        <h3 className="text-base font-semibold text-zinc-900">Disarankan untukmu</h3>
        <button
          type="button"
          onClick={onClose}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
            {recs.map((h: any) => (
              ...
              >
                ...
                <div className="line-clamp-1 text-sm font-semibold text-zinc-900">{h.name}</div>
...
function ChatRoleplayPanel({ slug, name, onClose }: { slug: string; name: string; onClose: () => void }) {
  const ask = HeroesApi.useChatAsHero();
...
[Interpretasi] — Aku akan menjawab dengan gaya naratif, namun tetap membedakan fakta dan interpretasi.`,
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const behavior: ScrollBehavior = loading ? "auto" : msgs.length > 1 ? "smooth" : "auto";
  ...
  async function send() {
    ...
    try {
      const payloadHistory: ChatMsg[] = [...msgs, { role: "user" as const, content: q }];
      const res = await ask({
        slug,
        message: q,
        history: payloadHistory,
      } as any);
      setMsgs((prev) => [...prev, { role: "assistant" as const, content: (res as any).reply }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="flex h-[640px] max-h-[85vh] w-[720px] max-w-[95vw] flex-col overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-black/10">
      <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
        <h3 className="text-base font-semibold text-zinc-900">Ngobrol dengan {name}</h3>
        <button
          type="button"
          onClick={onClose}
          className="cursor-pointer rounded-md border border-zinc-200 px-3 py-1.5 text-sm text-zinc-700 hover:bg-zinc-50"
...
function QuizPanel({ slug, onClose }: { slug: string; onClose: () => void }) {
  const generate = HeroesApi.useGenerateQuiz();
  const saveAttempt = HeroesApi.useSaveQuizAttempt();
  const [loading, setLoading] = useState(false);
  ...
  const correctCount = useMemo(() => {
    if (!quiz) return 0;
    return quiz.questions.reduce((acc, qq) => acc + (picked[qq.id] === qq.answerId ? 1 : 0), 0);
  }, [picked, quiz]);
    ...
    <div className="flex h-[560px] max-h-[85vh] w-[640px] max-w-[95vw] flex-col overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-black/10">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        {!loading && quiz && !done && q && (
          <div className="space-y-4">
            ...
            <h4 className="text-base font-semibold text-zinc-900">{q.prompt}</h4>
            ...
            {locked && q.explanation && (
              <div className="rounded-md bg-zinc-50 px-3 py-2 text-xs text-zinc-600 ring-1 ring-zinc-200">{q.explanation}</div>
            )}
          </div>
        )}
        {!loading && quiz && done && (
          <div className="space-y-4 text-center">
            ...
            {award && (
              <div className="mx-auto w-full max-w-md rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-900 ring-1 ring-amber-200">
                {award.practice ? (
                  <div>Mode latihan — tidak ada poin (kuota harian/hero sudah terpakai).</div>
                ) : (
                  <div className="space-y-1">
                    <div>
                      +{award.awardedPoints} poin {award.isPerfect && "(Perfect!)"}
                    </div>
                    ...
                    <div className="text-xs text-amber-800/80">Sisa kuis berbobot hari ini: {award.dailyRemaining}</div>
                  </div>
                )}
              </div>
            )}
            <div className="mx-auto w-full max-w-md rounded-md bg-blue-50 px-3 py-2 text-sm text-blue-900 ring-1 ring-blue-200">
              Ringkasan hasil kuis telah <span className="font-semibold">dikirim ke email</span> Anda.
              <span className="mt-1 block text-xs text-blue-800/80">Jika belum terlihat, periksa folder Spam/Promosi.</span>
            </div>
          </div>
        )}
      </div>
      ...
      {!loading && quiz && done && (
        <div className="flex items-center justify-center gap-2 border-t px-4 py-3">
          ...
          <Button className="cursor-pointer bg-yellow-400 text-black hover:bg-yellow-300" onClick={restart}>
            Coba Lagi
          </Button>
        </div>
      )}
...
function QuizModal({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  useEffect(() => {
    if (!open) return;
...
export function HeroDetail() {
  ...
  const hero = HeroesApi.useHeroBySlug(params?.slug) as HeroDoc | null | undefined;
  ...
  const bioSummary: string | undefined = hero?.biography?.summary ?? hero?.raw?.biography?.summary ?? hero?.summary;
  const bioHighlights: string[] = hero?.biography?.highlights ?? hero?.raw?.biography?.highlights ?? [];
  const [studentView, setStudentView] = useState<string | null>(null);
  const simplify = HeroesApi.useSimplifySummary();
  ...
  useEffect(() => {
    ...
    try {
      ...
      const next = [hero.slug, ...cur.filter((s) => s !== hero.slug)].slice(0, 10);
  ...
  if (!hero) {
    return (
      <div className="grid h-full w-full place-items-center">
        <div className="rounded-lg bg-white px-4 py-3 text-zinc-700 shadow">Data pahlawan tidak ditemukan.</div>
      </div>
    );
  }
    ...
    <>
      <div className="mb-3 hidden items-center sm:mb-4 sm:flex">
        <Breadcrumbs>
          ...
          <span className="text-neutral-400">{hero?.name ?? "Detail Pahlawan"}</span>
        </Breadcrumbs>
      </div>
      ...
      <div className="flex h-full min-h-0 w-full flex-col gap-4 md:grid md:grid-cols-5">
        <div className="relative col-span-1 hidden w-full overflow-hidden rounded-2xl ring-1 ring-black/10 md:col-span-2 md:block md:aspect-auto md:h-full">
          ...
          >
            {images.map((src, i) => (
              <div key={i} className="relative h-full w-full shrink-0 snap-center" style={{ minWidth: "100%" }}>
                <img
                  src={src}
                  alt={`${hero.name} ${i + 1}`}
          ...
          <button
            ...
            className={[
              ...
              idx === 0 ? "cursor-not-allowed opacity-30" : "cursor-pointer hover:bg-black/70",
            ].join(" ")}
          >
          ...
          <button
            ...
            className={[
              ...
              "bg-black/60 text-white shadow-md backdrop-blur transition",
              idx === images.length - 1 ? "cursor-not-allowed opacity-30" : "cursor-pointer hover:bg-black/70",
            ].join(" ")}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            ...
            {images.map((_, i) => (
              <button
                ...
                className={[
                  ...
                  i === idx ? "bg-white shadow" : "bg-white/50 hover:bg-white/80",
                  "cursor-pointer",
                ].join(" ")}
        ...
        <div className="relative col-span-1 flex min-h-0 flex-col rounded-2xl bg-white p-4 md:col-span-3 md:p-6">
          <div className="mb-4 hidden sm:flex sm:flex-col md:flex-row md:items-start md:justify-between">
            <h2 className="text-3xl font-bold tracking-tight text-zinc-900">{hero.name}</h2>
          ...
          <div className="mb-5 hidden flex-wrap gap-2.5 sm:flex">
            ...
            {Array.isArray(hero.raw?.aliases) && hero.raw.aliases.length > 0 && (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div className="mb-5 sm:hidden">
              <h2 className="text-center text-3xl font-bold tracking-tight break-words text-zinc-900">{hero.name}</h2>
            ...
            <div className="space-y-6">
              <section>
                <h3 className="mb-2 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Biografi Singkat</h3>
                {studentView ? (
                  <div className="space-y-2 text-[15px] leading-relaxed">
                    <p className="max-w-prose text-zinc-700">{studentView}</p>
                    <div className="text-xs text-zinc-500">
                      Tampilan pelajar — diringkas oleh AI.
                      <button type="button" className="ml-2 cursor-pointer underline" onClick={() => setStudentView(null)}>
                        Tampilkan versi asli
                      </button>
                    </div>
                  </div>
                ) : (
                  <p className="max-w-prose text-[15px] leading-relaxed text-zinc-700">{bioSummary ?? "—"}</p>
                )}
              </section>

              <section>
                <h3 className="mb-2 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Sorotan Perjuangan</h3>
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <section>
                  <h3 className="mb-1.5 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Kelahiran</h3>
                  <div className="text-[15px] leading-relaxed text-zinc-700">
                    <div>{fmtDate(hero.birth?.date)}</div>
                    <div className="text-zinc-500">{hero.birth?.place ?? "—"}</div>
                  </div>
                </section>

                <section>
                  <h3 className="mb-1.5 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Wafat</h3>
                  <div className="text-[15px] leading-relaxed text-zinc-700">
                    <div>{fmtDate(hero.death?.date)}</div>
                    <div className="text-zinc-500">{hero.death?.place ?? "—"}</div>
                  </div>
                </section>

                <section className="sm:col-span-2">
                  <h3 className="mb-1.5 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Pengakuan</h3>
                  <div className="text-[15px] leading-relaxed text-zinc-700">
                    <div>{hero.recognition?.basis ?? "—"}</div>
                    <div className="text-zinc-500">{fmtDate(hero.recognition?.date)}</div>
                  </div>
                </section>

                <section className="sm:col-span-2">
                  <h3 className="mb-1.5 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Pendidikan</h3>
                ...
                <section className="sm:col-span-2">
                  <h3 className="mb-1.5 text-sm font-semibold tracking-wide text-zinc-600 uppercase">Warisan</h3>
                  {hero.legacy?.length ? (
                    <ul className="list-disc space-y-2 pl-5 text-[15px] leading-relaxed text-zinc-700">
                      {hero.legacy.map((e, i) => (
                        <li key={i}>{e}</li>
          ...
          >
            ...
            <span className="hidden text-sm font-medium md:ml-2 md:inline">Yuk ngobrol dengan {hero.name}</span>
          </button>
        </div>
        ...
        <QuizModal open={chatOpen} onClose={() => setChatOpen(false)}>
          <ChatRoleplayPanel slug={hero.slug} name={hero.name} onClose={() => setChatOpen(false)} />
        </QuizModal>
      </div>
    </>
  );
