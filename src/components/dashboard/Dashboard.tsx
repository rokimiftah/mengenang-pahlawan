...
import { FilterProvider, useFilters } from "../filters/FilterContext";
import { SearchFilterLauncher } from "../filters/SearchFilterLauncher";
import { HeroDetail } from "../heroes/HeroDetail";
import { HeroShelf } from "../heroes/HeroShelf";
import { PointsBadge } from "../points/PointsBadge";
import { PointsHistoryModal } from "../points/PointsHistoryModal";
...
function HeaderSearchSlot() {
	...
	const handleApply = useCallback(
		(v: { q?: string }) => {
			...
			setFilters((prev) =>
				prev.q === trimmed ? prev : { ...prev, q: trimmed },
			);
		},
		[setFilters],
	);
				...
				<FilterProvider>
					<div className="flex h-dvh flex-col overflow-hidden bg-[#111110]">
						...
						<section className="relative mx-4 mb-4 flex flex-1 overflow-hidden rounded-lg bg-neutral-200 shadow-lg md:mx-8 md:mb-8">
							<div className="relative z-10 flex h-full min-h-0 w-full flex-col items-stretch p-4">
								{children}
							</div>
						</section>
					</div>
