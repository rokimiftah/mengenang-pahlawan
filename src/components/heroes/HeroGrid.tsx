// src/components/heroes/HeroGrid.tsx
...
export function HeroGrid() {
	...
	const heroes = useQuery(api.heroes.list, { q: filters.q, era: undefined });
	...
	const { cols, size, gap } = useMemo(() => {
		...
		const maxCols = Math.min(
			n,
			Math.max(1, Math.floor((W + GAP) / (MIN_REF + GAP))),
		);
		...
		if (best.size <= 0 || !Number.isFinite(best.size)) {
			const fallbackSize = Math.min(
				120,
				Math.max(72, Math.floor(Math.min(W, H) * 0.3)),
			);
			const fallbackCols = Math.max(
				1,
				Math.min(n || 1, Math.floor((W + GAP) / (fallbackSize + GAP))),
			);
